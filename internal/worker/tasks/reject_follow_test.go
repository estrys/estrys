...
func TestHandleRejectFollow(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err: "post to inbox was not accepted: error posting to inbox 401",
		},
		{
			name:      "ok",
			inputFile: "follow_ok",
			Mock: func(t *testing.T) {
				...
				fakeActivityPubClient.On(
					...
					fakeActor,
					fakeUser,
					mock.MatchedBy(func(act vocab.ActivityStreamsReject) bool {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			task := asynq.NewTask(
				tasks.TypeRejectFollow,
				payload,
			)

			err = tasks.HandleRejectFollow(context.TODO(), task)
			if tt.err != "" {
