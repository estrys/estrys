...
type AcceptFollowInput struct {
	Username string `json:"user"`
	Activity map[string]interface{}
}

func NewAcceptFollowTask(username string, act vocab.ActivityStreamsFollow) (*asynq.Task, error) {
	serializedActivity, err := streams.Serialize(act)
	if err != nil {
		return nil, err //nolint:wrapcheck
	}
	payload, err := json.Marshal(AcceptFollowInput{Username: username, Activity: serializedActivity})
	if err != nil {
		return nil, err //nolint:wrapcheck
	}
...
func HandleAcceptFollow(ctx context.Context, task *asynq.Task) error {
	...
	acceptFollow, err := vocabService.GetAccept(user, follow)
	if err != nil {
		return errors.Errorf("unable to create an accept request : %v: %s", err, asynq.SkipRetry)
	}
	...
	}).Info("sent accept follow to inbox")
	return nil
}
